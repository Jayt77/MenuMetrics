...
class UniversalDataAdapter:
    ...
    def map_columns(self, df: pd.DataFrame, profile: DatasetProfile,
                   target_schema: str = 'transaction_data') -> List[ColumnMapping]:
        """
        Intelligently map uploaded columns to standard schema.

        Uses fuzzy matching and pattern recognition to suggest mappings.
        ...
        """
        logger.info(f"Mapping columns to {target_schema} schema")

        if target_schema not in self.STANDARD_SCHEMAS:
            raise ValueError(f"Unknown schema: {target_schema}")

        schema = self.STANDARD_SCHEMAS[target_schema]
        mappings = []

        for standard_col, patterns in schema['column_patterns'].items():
            best_match = None
            best_confidence = 0
            ...
            for uploaded_col in df.columns:
                ...
                for pattern in patterns:
                    similarity = self._calculate_similarity(uploaded_col.lower(), pattern.lower())

                    if similarity > best_confidence:
                        best_confidence = similarity
                        best_match = uploaded_col

            if best_match and best_confidence > 0.5:  # Threshold for acceptance
                mappings.append(ColumnMapping(
                    uploaded_name=best_match,
                    standard_name=standard_col,
                    confidence=best_confidence
                ))
                logger.info(f"  Mapped: {best_match} -> {standard_col} (confidence: {best_confidence:.2f})")
    ...
    def validate_data(self, df: pd.DataFrame, mappings: List[ColumnMapping],
                     schema_type: str = 'transaction_data') -> ValidationResult:
        """
        Validate uploaded data against requirements.
        ...
        schema = self.STANDARD_SCHEMAS.get(schema_type, {})
        required_fields = schema.get('required', [])

        # Check required fields
        mapped_standard_cols = [m.standard_name for m in mappings]
        for required in required_fields:
            if required not in mapped_standard_cols:
                errors.append(f"Missing required field: {required}")

        # Check data quality
        null_pct = (df.isnull().sum().sum() / (len(df) * len(df.columns))) * 100
        if null_pct > 50:
            errors.append(f"High missing data: {null_pct:.1f}% of values are null")
        elif null_pct > 20:
            warnings.append(f"Moderate missing data: {null_pct:.1f}% of values are null")
        ...
        for optional in optional_fields:
            if optional not in mapped_standard_cols:
                if optional == 'unit_cost':
                    recommendations.append("Upload cost data for accurate profit analysis (currently estimating at 30%)")
                elif optional == 'customer_id':
                    recommendations.append("Upload customer data to enable segmentation analysis")
                elif optional == 'timestamp':
                    recommendations.append("Upload timestamps to enable time-based trend analysis")

        # Calculate quality score
        quality_score = 100
        quality_score -= len(errors) * 20  # Each error -20 points
        quality_score -= len(warnings) * 5  # Each warning -5 points
        quality_score = max(0, quality_score)

        is_valid = len(errors) == 0

        logger.info(f"Validation complete: valid={is_valid}, quality={quality_score}/100, "
